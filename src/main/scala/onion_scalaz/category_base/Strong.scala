...
trait Strong[=>:[_, _]] extends Profunctor[=>:] {
  ...
  def second[A, B, C](fa: A =>: B): (C, A) =>: (C, B) = {
    ...
    // 先变化 product中的first 得到新的Strong product形式
    val func1: (A, C) =>: (B, C) = first(fa)

    val func2: ((C, A)) => (A, C) = (ac: (C, A)) => ac.swap

    val func3: ((B, C)) => (C, B) = (bc: (B, C)) => bc.swap

    dimap(func1)(func2)(func3)
